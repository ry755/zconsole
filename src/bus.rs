...
impl Io for Bus {
    ...
    #[inline(always)]
    fn write_io(&mut self, port: u16, byte: u8, _ts: Self::Timestamp) -> (Option<()>, Option<NonZeroU16>) {
        ...
        match masked_port {
            ...
            _ => println!("[write_io] write to unmapped port: {:#X}, byte: {:#X}", masked_port, byte),
        }
...
impl Memory for Bus {
    ...
    fn read_mem16(&self, address: u16, _ts: Self::Timestamp) -> u16 {
        (self.memory.read_memory(address + 1) as u16) << 8 | (self.memory.read_memory(address) as u16)
    }
